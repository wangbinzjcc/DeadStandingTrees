...
Model <- function(parm, Data){
  ...
  size.priors <- dunif(size, max(Data$y), 50, log=T)
...
################
Initial.Values <- parm <- c(Prob11.logiT=logit(0.2911), 
                           Size=20, Prob22.logiT=logit(0.2911),
                           PropZeroF.logiT=logit(0.02))
#
MyData <- Data <- list( parm.names=names(parm), y=yy,
...
#################
#   write.csv(BF$Summary2, 'ZeroInflatedBinomial.distribution 20 m.csv')
############################ 
...
NB$Summary2
#################
# write.csv(NB$Summary2, 'ZeroInflatedNegativeBinomial.distribution 20m.csv')
############################################################################# 
...
#  write.csv(ZeroPois, "ZeroInflatedPoission.distribution.20m.csv")
