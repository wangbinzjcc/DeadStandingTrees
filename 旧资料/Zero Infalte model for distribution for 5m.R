...
#################
#   write.csv(FF2$Summary2, 'possion.distribution 5m.csv')
############################################################################
#   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~     #
#   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~     #
...
#################
#   write.csv(BF$Summary2, 'binomial.distribution 5 m.csv')
############################
Fit <- Fit2
...
NB$Summary2
#################
#   write.csv(NB$Summary2, 'NegativeBinomial.distribution 5m.csv')
############################
Fit <- Fit2
caterpillar.plot(Fit,Parms= c('BETA','RHO'))
#
...
NF$Summary2
#################
#   write.csv(NF$Summary2, 'nomial.distribution5m.csv')
...
hist(yy)  
#################################
Initial.Values <- parm <- c(lambda.log=log(1), prob=0.8)
MyData <- Data <- list(J=J, N=N, 
                       mon.names=c("lambda"),
...
Model <- function(parm, Data){
  ...
  Prob <- invlogit(parm[2])
  # log-likelihood
  y.ww <- as.numeric(Data$y > 0)
  ww.lambda <- y.ww *lambda
  #
  # priors distribution
  lambda.priors <- dgamma(lambda,15,log=T)
  prob.priors <- dunif()
  ##
  LL.ww <- sum(dbinom(x=y.ww, size=Size, prob=Prob, log=T))
  LL.cc <- sum(dpois(x=Data$y, lambda=ww.lambda,log=T))
  # log-Posterior
  LP <- LL.ww + LL.cc + lambda.pritor
  #    
  Modelout <- list(LP=LP, Dev=-2*LL,  Monitor=c(LP, lambda),  
                   yhat= rpois(n=Data$N, lambda = lambda) ,
                   parm=parm)
