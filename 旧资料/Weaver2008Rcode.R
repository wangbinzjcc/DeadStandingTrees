...
# #####
bay0 <- data.frame(x2=HT, x12=ST*HT, x11=ST^2, x22=HT^2) 
bay0$Ymat<-cbind(r0, n0-r0)
glm.bay<-glm(Ymat~x2+x12+x11+x22,family=binomial(link="logit"),data=bay0)
 summary(glm.bay)
# #####
 parm <- c(beta0 =-3.9,# beta1 =-0.425,
           beta2 =0.11, beta12 =-0.09, beta11 =-0.45, beta22 =-0.005)
 Data <- list(N= length(r0),mon.names=c("LogPosterior"), parm.names=names(parm),
...
for(i in 1:ndraws){
  pi0 = exp(beta0[i] + # beta1[i]*X1+
              beta2[i]*X2+beta12[i]*X1*X2+beta11[i]*X1^2+
           beta22[i]*X2^2)
  pi = pi0/(1+pi0)
...
exp0 = exp(beta0.mean+ # beta1.mean*X1+
           beta2.mean*X2+beta12.mean*X1*X2+
...
matrix.indices <- function(a,b){
 for(i in 1:nrow(b)){ #i is for rows
  ...
  for(i in 1:L1.n){
    for(j in 1:L2.n){
      exp0 = exp(beta0+ # beta1*L1.c[i]+
                   beta2*L2.c[j]+beta12*L1.c[i]*L2.c[j]+
...
#  R Code for Prediction
  ...
  exp0 = exp(beta0+#beta1*X1star+
               beta2*X2star+beta12*X1star*X2star+beta11*X1star^2+
        beta22*X2star^2)
  pi = exp0 / (1+exp0)
